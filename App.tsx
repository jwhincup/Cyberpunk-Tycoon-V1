...
import { getClickerUpgradeInfo } from './src/gameLogic/clickerUpgrades';
...
const App: React.FC = () => {
    ...
    const handleGameTick = () => {
        ...
        setGameState(prev => {
            ...
            const newStocks = prev.stocks.map(stock => {
                ...
                const sineWave = Math.sin(cycle * stock.simulationFrequency);
                const priceOffset = sineWave * stock.simulationAmplitude;
                const randomNoise = (Math.random() - 0.5) * stock.volatility * stock.basePrice;
                ...
                const basePriceWithFluctuations = stock.basePrice + priceOffset + randomNoise;
    ...
    const tradeItem = (id: string, quantity: number) => {
        setGameState(prev => {
            ...
            if (!isBuying && item.id === prev.assignedCarId && item.owned + quantity <= 0) {
                newAssignedCarId = null;
            }
            
            if (isBuying && item.effect.type === 'UNLOCK_MISSIONS') {
                if (item.owned > 0) return prev;

                const newUnlockedMissions = new Set(unlockedMissions);
                MISSION_TEMPLATES.filter(t => t.isLocked).forEach(t => newUnlockedMissions.add(t.id));
                setUnlockedMissions(newUnlockedMissions);

                const newBalance = prev.balance - tradeCost;
                const newItems = prev.items.map(i =>
                    i.id === id ? { ...i, owned: 1 } : i
                );
                return { ...prev, balance: newBalance, items: newItems };
            }

            const newBalance = prev.balance - tradeCost;
            const newItems = prev.items.map(i =>
                i.id === id ? { ...i, owned: i.owned + quantity } : i
            );
            
            return { ...prev, balance: newBalance, items: newItems, assignedCarId: newAssignedCarId };
        });
    };

    const purchaseAuctionProperty = (id: string) => {
        setGameState(prev => {
            const prop = prev.flippableProperties.find(p => p.id === id);
            if (!prop || !prop.forSale || prop.owned) return prev;

            const finalCost = prop.auctionPrice * getCostReduction(prev);
            if(prev.balance < finalCost) return prev;

            const newBalance = prev.balance - finalCost;
            const newProperties = prev.flippableProperties.map(p =>
                p.id === id ? { ...p, owned: true, forSale: false } : p
            );

            return { ...prev, balance: newBalance, flippableProperties: newProperties };
        });
    };

    const upgradeProperty = (id: string, improvementType: ImprovementType) => {
        setGameState(prev => {
             const newProperties = prev.flippableProperties.map(p => {
                if (p.id !== id) return p;

                const improvement = p.improvements[improvementType];
                if (improvement.level >= improvement.maxLevel) return p;

                const cost = improvement.baseCost * Math.pow(1.5, improvement.level);
                 if (prev.balance < cost) return p;

                const newBalance = prev.balance - cost;
                const newImprovement = { ...improvement, level: improvement.level + 1 };
                const newImprovements = { ...p.improvements, [improvementType]: newImprovement };
                
                setGameState(innerPrev => ({...innerPrev, balance: newBalance}));
                return { ...p, improvements: newImprovements };
            });

            return { ...prev, flippableProperties: newProperties };
        });
    };
    ...
    const sellProperty = (id: string) => {
        setGameState(prev => {
            const prop = prev.flippableProperties.find(p => p.id === id);
            if (!prop || !prop.owned) return prev;
            
            const salePrice = prop.baseValue * 1.2;

            const newBalance = prev.balance + salePrice;
            const newProperties = prev.flippableProperties.map(p => 
                p.id === id ? {...p, owned: false, isRented: false, forSale: true } : p
            );
    ...
    const startMission = (missionId: string) => {
        setGameState(prev => {
            ...
            if (!mission || mission.status !== 'AVAILABLE') return prev;

            const newMissions = prev.missions.map(m => 
                m.id === missionId ? { ...m, status: 'IN_PROGRESS' as const, timeLeft: m.duration } : m
            );
            return { ...prev, missions: newMissions };
        });
    };

    const hirePersonnel = (id: string, type: 'staff' | 'security') => {
        setGameState(prev => {
            const personnelList = type === 'staff' ? prev.staff : prev.security;
            const person = personnelList.find(p => p.id === id);
            if (!person) return prev;

            const finalCost = (person.cost * Math.pow(person.costMultiplier, person.owned)) * getCostReduction(prev);
            if (prev.balance < finalCost) return prev;
            ...
            return {
                ...prev,
                balance: prev.balance - finalCost,
                ...(type === 'staff' ? { staff: newPersonnelList } : { security: newPersonnelList })
            };
        });
    };

    const assignCompanyCar = (itemId: string) => {
        setGameState(prev => {
            const item = prev.items.find(i => i.id === itemId);
            if (!item || item.type !== 'car' || item.owned <= 0) return prev;
            return { ...prev, assignedCarId: itemId };
        });
    };

    const executeMerger = (mergerId: string) => {
        setGameState(prev => {
            const merger = prev.mergers.find(m => m.id === mergerId);
            if (!merger || merger.status === 'COMPLETED') return prev;
            
            if (!checkRequirements(merger.requirements, prev)) {
                return prev;
            }

            let newBalance = prev.balance;
            let newBusinesses = [...prev.businesses];

            for (const req of merger.requirements) {
                 switch(req.type) {
                    case 'balance': newBalance -= req.quantity; break;
                    case 'business': 
                        newBusinesses = newBusinesses.map(b => b.id === req.id ? {...b, owned: 0} : b);
                        break;
                }
            }
            
            if (merger.result.type === 'unlock_business') {
                newBusinesses = newBusinesses.map(b => b.id === merger.result.id ? {...b, unlocked: true, hidden: false } : b);
            }

            const newMergers = prev.mergers.map(m => m.id === mergerId ? {...m, status: 'COMPLETED' as const} : m);

            return {...prev, balance: newBalance, businesses: newBusinesses, mergers: newMergers};
        });
    };

    const handlePrestige = () => {
        if (!window.confirm(
            "You have amassed a trillion credits. The Syndicate is relocating you to a new system to expand the intergalactic effort. Your assets will be liquidated to fund the journey, but your experience will grant you a permanent 10% income bonus on all future ventures.\n\nAre you ready to ascend?"
        )) {
            return;
        }

        setGameState(prev => {
            if (prev.balance < PRESTIGE_REQUIREMENT) return prev;
    ...
    const loadFromCode = (code: string) => {
        try {
            ...
            if (typeof loadedState.balance !== 'number' || !Array.isArray(loadedState.businesses)) {
                throw new Error('Invalid save code: Missing essential properties.');
            }
            ...
            alert("Invalid or corrupt save code!");
        }
    };
    ...
    const renderPanel = () => {
        ...
        switch (activePanel) {
            case Panel.Businesses:
                return <BusinessPanel businesses={gameState.businesses} balance={gameState.balance} onPurchase={purchaseBusiness} onPurchaseUpgrade={purchaseBusinessUpgrade} />;
            case Panel.Upgrades:
                return <UpgradesPanel upgrades={gameState.upgrades.filter(u => u.category === 'wetware')} balance={gameState.balance} onPurchase={purchaseUpgrade} />;
            case Panel.CompanyUpgrades:
                return <CompanyUpgradesPanel 
                    upgrades={gameState.upgrades.filter(u => u.category === 'corporate')} 
                    missions={gameState.missions}
                    balance={gameState.balance} 
                    onPurchaseUpgrade={purchaseUpgrade} 
                    onStartMission={startMission}
                    isLocked={!isCorpUnlocked}
                    gameState={gameState}
                    onHirePersonnel={hirePersonnel}
                    onAssignCar={assignCompanyCar}
                    onExecuteMerger={executeMerger}
                />;
            case Panel.Projects:
                return <ProjectsPanel projects={gameState.projects} balance={gameState.balance} onStart={startProject} />;
            case Panel.StockMarket:
                return <StockMarketPanel 
                    stocks={gameState.stocks} 
                    balance={gameState.balance} 
                    onTradeStock={tradeStock}
                    autoTraderUnlocked={gameState.autoTraderUnlocked}
                    onUpdateAutoTraderSettings={handleUpdateAutoTraderSettings}
                    marketStatus={gameState.marketStatus}
                    activeNews={gameState.activeNews}
                />;
            case Panel.Market:
                return <MarketPanel items={gameState.items} balance={gameState.balance} onTrade={tradeItem} contrabandLimit={gameState.contrabandLimit} />;
            case Panel.RealEstate:
                return <RealEstatePanel 
                    properties={gameState.flippableProperties}
                    balance={gameState.balance}
                    onPurchase={purchaseAuctionProperty}
                    onUpgrade={upgradeProperty}
                    onSell={sellProperty}
                />;
            case Panel.Stats:
                 return <StatsPanel gameState={gameState} onPrestige={handlePrestige} />;
            default:
                return null;
        }
    };
    ...
    const corpShellProject = gameState.projects.find(p => p.id === 'proj2');
    const isCorpUnlocked = corpShellProject?.status === 'COMPLETED';
    const currentPlanet = PLANETS[gameState.planetsVisited % PLANETS.length];

    return (
        <div className="min-h-screen bg-cyber-bg bg-[radial-gradient(circle_at_center,_rgba(38,38,83,0.4)_0%,_rgba(10,10,26,0.9)_100%)] p-4 md:p-8 flex flex-col items-center font-rajdhani text-cyber-text">
            {isDevMode && <DevPanel gameState={gameState} onLoad={loadFromCode} onClose={() => setIsDevMode(false)} />}
            <div className="w-full max-w-7xl mx-auto">
                <Header balance={gameState.balance} ips={totalIPS} planetName={currentPlanet.name} onTitleClick={() => setIsDevMode(true)} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                    <div className="lg:col-span-1 flex flex-col gap-6">
                       <Clicker 
                            ...
                            level={gameState.clickerLevel}
                            balance={gameState.balance}
                            onUpgrade={purchaseClickerUpgrade}
                            nextUpgradeInfo={getClickerUpgradeInfo(gameState.clickerLevel)}
                        />
                    </div>
